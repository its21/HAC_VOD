...
Allowed maximum 5 pages in total.\\
..\\
..\\
..\\
..\\
   ...
   \begin{figure}
	...
	\includegraphics[scale=0.398]{vtot_r.pdf}
		       	       \includegraphics[scale=0.398]{E_L.pdf}
		       	       	       	       ...
		       	       	       	       \vspace{-0.4cm}
   \caption{Total velocity versus galactocentric distance for VOD and HAC, showing in red the sub group VOD (11-16 kpc). Energy versus angular momentum and angular momentum only for stars with 11-16 kpc galactocentric distance. The energy here is the galpy energy minus the potential at infinity (will do what Sarah did too and see how it changes).}
    \label{fig:energy}
\end{figure}
%
\subsection{Properties of the orbits}
We use the 6-D measurements described in previous sub-section to initialise the  RR Lyrae orbits in the HAC and VOD fields. We integrate them with the $\mathrm{galpy}$ package \citep{Bovy2015} in the recommended Galactic potential model for the Milky Way. $\mathrm{MWPotential2014}$ is composed of a Miyamoto-Nagai disc, a bulge with a power-law density profile that is exponentially cut-off and a dark matter halo described by a NFW potential, with parameters given in table 1, \citet{Bovy2015}. The orbit characteristics, i.e. the peri- and apocenter radii, eccentricity, and the maximal height above the orbit plane, are shown in Fig \ref{fig:orbits}. To compute the errors (not shown for VOD to simply the figure) we integrated 500 orbits for each star where the orbits were initialised on parameters resampled from the observations. The pdfs of the eccentricities, apocentres and pericentres are shown in the bottom panels.
If the HAC orbits are clearly highly eccentric, we find that the majority of stars in VOD are also on quite eccentric orbits: in particular, the stars with $11<\mathrm{r_{GC}}$ (kpc)$<16$ (in red), follow closely the HAC orbit characteristics.% (need to expand here)
Note that the default mass value in $\mathrm{galpy}$, $\mathrm{M_{vir}} = 0.8 \times 10^{12} M_{\odot}$, is in disagreement with more recent measurements which place it at double the value (to add citations), however the orbital parameters are minimally affected. %I should actually plot fig4 to check, the mass made a small difference on Figure 5.

For completeness, in Figure \ref{fig:energy} we show the distribution of the total velocity ($\mathrm{v_{tot}^{2}} = \mathrm{v_{r}}^2+\mathrm{v_{\theta}}^2+\mathrm{v_{\phi}}^2$) as a function of galactocentric radius for all RR Lyrae, and energy and angular momentum distributions only for the stars with $11\mathrm{<r_{GC}}/$kpc$<16$. %more to add if we keep this

%
\subsection{Are the HAC and VOD related?}
...
\begin{figure}
	     \includegraphics[scale=0.302]{HAC_orbits_8Gyrs_lb_defaultmass.pdf}
	     	     ...
	     	     \includegraphics[scale=0.302]{VOD_orbits_8Gyrs_xy_defaultmass.pdf}
	     	     	     \includegraphics[scale=0.302]{VOD_orbits_8Gyrs_yz_defaultmass.pdf}\\
	     	     	     	     \includegraphics[scale=0.302]{VOD_orbits_8Gyrs_lb_sausage.pdf}
