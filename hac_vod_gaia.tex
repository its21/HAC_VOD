...
The velocity distribution in the VOD and HAC fields is shown in
Fig. \ref{fig:vel} in spherical polar coordinates, where $v_{r}$,
$v_{\theta}$, $v_{\phi}$ are the radial, azimuthal and polar
components respectively. `Group 2' stars (lime) form a group at $v_{r}
= 135$ km/s as expected, while the stars at intermediate
$\mathrm{r_{GC}}$ (red) seem to have a velocity distribution very
similar to HAC, shown in the top panels. To estimate the error on each
velocity component we resample the data 1000 times from a multivariate
Gaussian distribution with mean the measurement \{$\alpha$, $\delta$,
$\mathrm{d_{helio}}$, $\mu_{\alpha} \mathrm{cos(\delta)}$,
$\mu_{\delta}$, $\mathrm{v_{helio}}$\}$_{i}$ and full covariance
matrix which takes into account the covariances between the right
ascension $\alpha$, declination $\delta$ and proper motions, provided
by GDR2. We take the standard deviation of the resulting \{$v_{r}$,
$v_{\theta}$, $v_{\phi}$\} distributions as the upper limit of the
velocity uncertainties and show them in Fig. \ref{fig:vel}.
