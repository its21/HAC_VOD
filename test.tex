...
\begin{figure}
	...
	% Allowable file formats are eps or ps if compiling using latex
	% or pdf, png, jpg if compiling using pdflatex
	       \includegraphics[scale=0.35]{VOD_orbits_500Myrs_meanHAC.pdf}
	       	       \includegraphics[scale=0.35]{VOD_orbits_500Myrs_meanVOD.pdf}
	       	       \includegraphics[scale=0.52]{VOD_orbits_12Gyrs.pdf}
	       	       	       	       \includegraphics[scale=0.52]{VOD_orbits_12Gyrs_doublemass.pdf}
   \caption{\textit{Still to decide what to keep. }The backward orbit integration for HAC (left panels) and VOD (right panels) for 500 Myrs look back time on the top 2 rows and 12 Gyrs on bottom 2 rows. On the last row  $M_{vir} = 1.6 \times 10^{12} M_{\odot}$ instead of $M_{vir} = 0.8 \times 10^{12} M_{\odot}$, the default galpy value used for all the other figures. The HAC panels have VOD isodensity contours and viceversa. log(N) shown. }
    \label{fig:orbitslb}
\end{figure}
